...
void solve(){
    int n;cin>>n;
    unordered_map<int,int>mp1,amp1;
    unordered_map<int,int>mp2,amp2;
    vector<int>row,col;
    ...
    for(int i=0;i<row.size();i++)mp1[row[i]]=i+1;
    for(int i=0;i<col.size();i++)mp2[col[i]]=i+1;
    ...
    for(int i=2;i<=row.size();i++){
        for(int j=2;j<=col.size();j++){
            ...
            // cout<<i<<' '<<j<<' '<<pre[i][j]<<endl;
            int h=row[i-1]-row[i-2],l=col[j-1]-col[j-2];
            ans[pre[i][j]]+=h*l%mod;
            ans[pre[i][j]]%=mod;
