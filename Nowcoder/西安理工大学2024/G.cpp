...
int dfs(int i,int is_limit,int is_num,int cnt,int pre){
    ...
    int res=0;
    if(!is_num)res+=dfs(i-1,false,false,cnt,pre);
    int up=9;
    if(is_limit)up=nums[i];
    if(is_num){
        int j=pre+2;
        if(j<=up)res+=dfs(i-1,is_limit&&j==up,true,cnt+1,j);
        j=pre-2;
        if(j>=0&&j<=up)res+=dfs(i-1,is_limit&&j==up,true,cnt+1,j);
    }else{
        for(int j=1;j<=up;j++){
            res+=dfs(i-1,is_limit&&j==up,true,cnt+1,j);
        }
    }
